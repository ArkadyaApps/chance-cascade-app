      ...
      >
        <CarouselContent className="-ml-2 md:-ml-4">
          {featuredProducts.map((product) => (
            <FeaturedCarouselItem key={product.id} product={product} />
          ))}
        </CarouselContent>
...
const FeaturedCarouselItem = ({ product }: FeaturedCarouselItemProps) => {
  const navigate = useNavigate();
  const { t } = useTranslation();
  const cardRef = useRef<HTMLDivElement>(null);
  const imageRef = useRef<HTMLDivElement>(null);
  
  const cardParallax = useMouseParallax({ ref: cardRef, intensity: 10 });
  const imageParallax = useMouseParallax({ ref: imageRef, intensity: 15 });
  ...
  return (
    <CarouselItem className="pl-2 md:pl-4 basis-[85%] md:basis-1/2 lg:basis-1/3">
      <Card 
        ref={cardRef}
        className="overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-xl border-primary/20"
        style={cardParallax}
        onClick={() => navigate(`/product/${product.id}`)}
      >
        <div ref={imageRef} className="relative h-48 overflow-hidden bg-gradient-to-br from-primary/5 to-accent/5">
          <div
            style={imageParallax}
